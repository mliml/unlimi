<template>
  <div class="bg-white rounded-2xl lg:p-6 p-4 shadow-sm h-full flex flex-col">
    ...
    <!-- Week Days -->
    <div class="flex justify-between lg:gap-2 gap-1 mb-4">
      <button
        v-for="day in weekDays"
        :key="day.date"
        @click="selectedDay = day.date"
        :class="[
          'flex flex-col items-center lg:p-2 p-1.5 rounded-xl transition-all duration-200',
          selectedDay === day.date
            ? 'bg-tertiary text-white scale-110 shadow-md'
            : 'hover:bg-gray-100 text-textSub'
        ]"
      >
        <span class="lg:text-xs text-[10px] font-medium font-serif">{{ day.day }}</span>
        <span :class="[
          'lg:text-lg text-base font-bold font-serif mt-1',
    ...
    <div class="flex-1 overflow-y-auto space-y-3 mb-4">
      ...
      <!-- Empty State -->
      <div v-if="displayActivities.length === 0" class="flex flex-col items-center justify-center h-full text-gray-400">
...
const displayActivities = computed(() => {
  const all = todayActivities.value
  return isMobile.value ? all.slice(0, 3) : all
})
